...
const state = {
    ...
    seg: 'gcamp',  // which segmentation: 'gcamp' or 'dapi'
...
// Each segmentation's data lives in a sibling folder. We serve from the Max/ parent
// dir, so the combined dashboard reaches them via relative paths. The colour scheme is
// shared (only stored in the gcamp folder).
const SEG_DIRS = {
    gcamp: '../max_dashboard/data',
    dapi: '../max_dashboard_dapi/data'
};
const COLOUR_PATH = '../max_dashboard/data/cell_colour_scheme_yao.json';
function DATA() { return SEG_DIRS[state.seg] || SEG_DIRS.gcamp; }
...
const PARAM_CONFIG = {
    ...
    rTheta: {
        default: 1000000,
        values: [2, 50, 100, 1000000],
        displayName: 'rTheta'
    },
    mrfBeta: {
        default: 0,
        values: [0, 1],
        displayName: 'mrf_beta'
    },
    insideCellBonus: {
        default: 5,
        values: [0, 5],
        displayName: 'InsideCellBonus'
    }
};

// Run mapping: configuration -> run_id  (key = paramName:radioValueString)
const RUN_MAPPING = {
    'default': 'run_0',
    // Inefficiency
    'inefficiency:0.01': 'run_1',
    'inefficiency:0.05': 'run_2',
    'inefficiency:0.1': 'run_3',
    // rRho
    'rRho:1': 'run_4',
    'rRho:1000': 'run_5',
    'rRho:10000': 'run_6',
    ...
    'mrfBeta:1.5': 'run_12',
    // InsideCellBonus
    'insideCellBonus:0': 'run_13',
    'insideCellBonus:2': 'run_14',
    'insideCellBonus:10': 'run_15'
};

// Region display names (Max's tile has no spatial regions -> single "all")
const REGION_NAMES = {
    'all': 'All cells'
};

// Region to representative cell class mapping (purity chart; not meaningful for a single region)
const REGION_COLORS = {
    'all': 'Whrn'
};
...
const SIMPLE_CLASS_COLORS = {
    'Whrn': '#1f77b4',       // blue
    'C1ql2Ptgfr': '#d62728', // red
    'Batf3': '#2ca02c',      // green
    'Hsd11b1': '#9467bd',    // purple
    'Krt80': '#ff7f0e',      // orange
    'Cdh13': '#17becf',      // teal
    'Chrna6': '#bcbd22',     // olive
    'Other': '#999999',      // grey
...
async function init() {
    try {
        ...
        // Build combo map: "inef|rRho|rTheta|mrf|bonus" -> run id (multi-knob lookup)
        rebuildComboMap(state.metadata);
        // Keep refreshing while the 512-run grid is still computing
        startGridPolling();

        // Load base run data
        state.baseData = await loadJSON(`${DATA()}/${state.metadata.base_run}.json`);

        // Load color scheme (shared across segmentations)
        state.cellColors = await loadJSON(COLOUR_PATH);
        ...
        populateSpatialRunSelector();
            ...
            if (selectedRunIdEl && selectedRunDisplayEl) {
                // show the seg too, so the box changes when you flip gcamp <-> dapi
                // (run ids are the same in both grids, only the data behind them differs)
                selectedRunIdEl.textContent = `${state.seg} / ${state.currentRun}`;
        ...
        async function loadJSON(path) {
            // Prefer the inlined per-segmentation bundle so the dashboard runs from file://
            // (no fetch, no CORS). Falls back to fetch when served over http.
            const bundle = (state.seg === 'dapi') ? window.DASH_DAPI : window.DASH_GCAMP;
            // colour scheme is shared; it only lives in the gcamp bundle
            if (path.includes('cell_colour_scheme_yao.json')) {
                const cb = window.DASH_GCAMP || bundle;
                if (cb && cb.cellColors) return cb.cellColors;
            } else if (bundle) {
                if (path.includes('runs_metadata.json')) return bundle.metadata;
                const m = path.match(/(run_\d+)\.json/);
                if (m && bundle.runs && bundle.runs[m[1]]) return bundle.runs[m[1]];
                // run not in this bundle (e.g. dapi combo not computed yet): fall through to
                // fetch, which works over http and throws under file:// (handled as no-data)
            }
...
// Set up event listeners
// Switch segmentation (gcamp <-> dapi): reload metadata, base and the current selection
// from the other data folder, then redraw with whatever 5-knob combo is currently picked.
async function onSegChange(seg) {
    if (seg === state.seg) return;
    state.seg = seg;
    try {
        showLoading(true);
        state.metadata = await loadJSON(`${DATA()}/runs_metadata.json`);
        ...
        state.altData = null;
        applyCombo();  // reloads the matching run for this seg and redraws
        showLoading(false);
    } catch (e) {
        console.error('Failed to switch segmentation to', seg, e);
...
function setupEventListeners() {
    // Segmentation toggle (gcamp / dapi)
    document.querySelectorAll('input[name="segmentation"]').forEach(radio => {
        radio.addEventListener('change', (event) => {
            if (event.target.checked) onSegChange(event.target.value);
        });
    });

    // Parameter radio buttons
    const parameterGroups = ['inefficiency', 'rRho', 'rTheta', 'mrfBeta', 'insideCellBonus'];
        ...
        radios.forEach(radio => {
            radio.addEventListener('change', (event) => {
                if (event.target.checked) {
                    // Multi-knob mode: read ALL five buttons and load the matching combo.
                    applyCombo();
                }
            });
        });
...
function getCurrentConfig() {
    ...
    const rRhoRadio = document.querySelector('input[name="rRho"]:checked');
    const rThetaRadio = document.querySelector('input[name="rTheta"]:checked');
    const mrfBetaRadio = document.querySelector('input[name="mrfBeta"]:checked');
    const insideCellBonusRadio = document.querySelector('input[name="insideCellBonus"]:checked');

    config.inefficiency = inefficiencyRadio ? parseFloat(inefficiencyRadio.value) : PARAM_CONFIG.inefficiency.default;
    config.rRho = rRhoRadio ? parseFloat(rRhoRadio.value) : PARAM_CONFIG.rRho.default;
    config.rTheta = rThetaRadio ? parseFloat(rThetaRadio.value) : PARAM_CONFIG.rTheta.default;
    config.mrfBeta = mrfBetaRadio ? parseFloat(mrfBetaRadio.value) : PARAM_CONFIG.mrfBeta.default;
    config.insideCellBonus = insideCellBonusRadio ? parseFloat(insideCellBonusRadio.value) : PARAM_CONFIG.insideCellBonus.default;
...
// ---- Multi-knob combo mode (live backend) ----
const COMBO_AXES = ['inefficiency', 'rRho', 'rTheta', 'mrfBeta', 'insideCellBonus'];
const BASE_STR = { inefficiency: '0.001', rRho: '1000000', rTheta: '1000000', mrfBeta: '0', insideCellBonus: '5' };

function readComboKey() {
    return COMBO_AXES.map(n => {
        const r = document.querySelector(`input[name="${n}"]:checked`);
        return r ? r.value : '';
    }).join('|');
}

// Rebuild combo map from metadata (used at init and on each poll)
function rebuildComboMap(meta) {
    const map = {};
    (meta.runs || []).forEach(run => {
        if (run.params) map[COMBO_AXES.map(n => run.params[n]).join('|')] = run.id;
    });
    state.comboMap = map;
    state.gridDone = Object.keys(map).length;
}
...
function startGridPolling() {
    ...
    const timer = setInterval(async () => {
        try {
            ...
            state.metadata = m;
            rebuildComboMap(m);
            const el = document.getElementById('selectedRunId');
            if (el && el.textContent.startsWith('not computed')) {
                el.textContent = `not computed (${state.gridDone}/${TOTAL} done)`;
            }
            // if the current selection just became available, render it
            const key = readComboKey();
            if (state.comboMap[key] && state.currentRun !== state.comboMap[key]) applyCombo();
            if (state.gridDone >= TOTAL) clearInterval(timer);
        } catch (_) {}
    }, 15000);
}

// Look up and load the precomputed run for the current 5-button selection
async function applyCombo() {
    const key = readComboKey();
    const runId = (state.comboMap && state.comboMap[key]) || null;
    ...
    try {
        showNoDataMessage(false);
        if (runId === state.metadata.base_run) {
            state.altData = state.baseData;
        } else if (runId !== state.currentRun || state.altData === state.baseData) {
        ...
        console.error('Failed to load combo run', runId, e);
        showNoDataMessage(true);
    }
}
...
async function onParameterChange(paramName, paramValueStr, paramValueNum) {
    ...
    const selectedRunDisplayEl = document.getElementById('selectedRunDisplay');
    if (selectedRunIdEl && selectedRunDisplayEl) {
        selectedRunIdEl.textContent = `${state.seg} / ${runId}`;
        selectedRunDisplayEl.classList.remove('hidden');
    }
...
// Stable color per full class name: use the loaded scheme if present, else hash to a hue
function classColor(name) {
    if (name === 'Zero') return '#000000';
    if (state.cellColors && state.cellColors[name]) {
        const c = state.cellColors[name];
...
function renderSankeyChart() {
    ...
    // Build the class list dynamically from this run's transitions (full class names)
    const present = new Set();
    const totals = {};
    transitions.forEach(t => {
        present.add(t.from); present.add(t.to);
        totals[t.from] = (totals[t.from] || 0) + t.count;
    });
    // order by total outgoing flow (desc), Zero pinned to the bottom
    const CLASSES = Array.from(present).sort((a, b) => {
        if (a === 'Zero') return 1;
        if (b === 'Zero') return -1;
    ...
    const nodeLabels = [
        ...CLASSES.map(c => `${c} (Base)`),
        ...CLASSES.map(c => `${c} (Alt)`)
    ];
    const classToBeforeIdx = {};
    const classToAfterIdx = {};
    CLASSES.forEach((c, i) => { classToBeforeIdx[c] = i; classToAfterIdx[c] = i + NC; });
    ...
    const nodeColors = [...CLASSES, ...CLASSES].map(c => classColor(c));
...
function simplifyClassLabel(label) {
    // Full class names now: pass through unchanged.
    return (label && typeof label === 'string') ? label : 'Zero';
}
